...
app.controller('game2Ctrl', function ($scope, $state, bddFactory, $ionicPopup) {
	...
	*/
	...
	$scope.answerRight = function answerRight(u) {
		...
		if($scope.remaining > 0){
			if (i<6) {
				...
				if ($scope.question.answers[i].localeCompare($scope.question.question.french) == 0 && u==1) {
					...
					i++;
					message = {
						template: ' <p style="font-weight : bold; border-bottom : 1px red solid;";> <span class="icon ion-checkmark-round" style = "color: green; font-weight : 60px; font-size : 30px;"/></span> ' + 'Well, that wasn\'t the good answer ;-)'  + '</p> <br/><p> <span style="text-decoration: line-through;"">',
				...
				$scope.score = $scope.score -1;
				message = {
					template: ' <p style="font-weight : bold; border-bottom : 1px red solid;";> <span class="icon ion-close-round" style = "color: red; font-weight : 60px; font-size : 30px;"/></span> ' + 'That wasn\'t the good answer :-('  + '</p> <br/><p> <span style="text-decoration: line-through;"">',
	    ...
	    if(JSON.parse( window.localStorage.getItem('score_game_2')) == null || window.localStorage.getItem('score_game_2') < $scope.score)
	    {

	        window.localStorage.setItem('score_game_2', JSON.stringify($scope.score));
